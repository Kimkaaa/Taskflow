...
function DeleteButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="inline-flex h-10 w-20 cursor-pointer items-center justify-center gap-2 rounded-xl border border-red-500/30 text-sm font-semibold text-red-200 transition disabled:cursor-wait disabled:opacity-80"
    >
      {pending ? (
        <LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" />
      ) : (
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <form
                ...
                onSubmit={(event) => {
                  if (
                    !window.confirm(
                      "초대 링크를 삭제할까요? 기존 링크는 더 이상 사용할 수 없습니다.",
                    )
                  ) {
